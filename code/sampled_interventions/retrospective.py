...
from finite_catalog.certificate import exact_finite_robust
from sampled_interventions.methods import paired,generic_enumerated,summary
...
def main():
 ...
 paths={p.parent.name:p for p in (R.parent/'finite_catalog/artifacts/qa_generation').glob('*/predictions.npz')};f=dict(np.load(paths['baseline_0']));names=sorted(set(paths)-{'baseline_0'})
 for metric in ['f1','em']:
  for pol in names:
