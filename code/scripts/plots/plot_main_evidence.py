...
parser.add_argument('--planner-summary', type=Path,
                    default=ROOT/'planning/results/summary.json')
parser.add_argument('--thresholds', type=Path,
                    default=ROOT/'pairing_analysis/results/dense_thresholds.csv')
parser.add_argument('--output-dir', type=Path, default=ROOT/'outputs/figures')
args = parser.parse_args()
source = args.planner_summary
thresholds = args.thresholds
for path in (source, thresholds):
    if not path.is_file():
        parser.error(f'missing input: {path}; see README for required result files')
...
dest=args.output_dir;dest.mkdir(exist_ok=True, parents=True)
for suffix in ['pdf','svg','png']:
    fig.savefig(str(dest/('audit_evidence.'+suffix)),dpi=220)
plt.close(fig)
manifest={'inputs':{'planner_summary':str(source.resolve()),
...
(dest/'audit_evidence_inputs.json').write_text(json.dumps(manifest,indent=2),encoding='utf-8')
