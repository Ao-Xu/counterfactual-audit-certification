...
from sampled_interventions.methods import kl_radius,eb
from shared_anchor.methods import shared_stats
from certificate_efficiency.run import kl_upper,wilson
...
def run(phase):
    ...
    counts={};covered={};family_false={};width_sum={};width_sq={}
    def record(key,u):
      if phase!='plan' and key not in allowed:return
      counts.setdefault(key,np.zeros(3,dtype=int));counts[key]+=u<0
      covered.setdefault(key,np.zeros(3,dtype=int));covered[key]+=u>=-P['robust_margin']
      family_false[key]=family_false.get(key,0)+int(np.any(u+P['robust_margin']<0))
      width=u+P['robust_margin']
      width_sum.setdefault(key,np.zeros(3));width_sum[key]+=width
      width_sq.setdefault(key,np.zeros(3));width_sq[key]+=width*width
    locked=[]
    if phase=='confirm':
    ...
    for rep in range(reps):
      ...
      for L in P['nuisance_support']:
       ...
       for K in P['K']:
        ...
        for N in Ns:
          pm,pcov,pd=pooled[N];ym=(gm@pm+offset)[:,None]+(qc@pm)[:,None]/(4*ETAS)
          ...
          gu=up.min(1)
          shu=shared_stats(gm@pm+offset,mv,qc@pm,N,P['rho'],P['delta'],P['J'])['upper']
          ep=P['delta']/(4*P['J'])
          matched=gm@pm+offset+kl_radius(project(gm,pd),N,N,2,ep)+np.sqrt(P['rho']*np.minimum(1,kl_upper(qc@pm,N,2,ep)))
          for local,geo in enumerate(ids):
           sl=slice(3*local,3*local+3)
           record((geo,'generic_DRO_EB',N,0,N,K),gu[sl])
           record((geo,'shared_moment',N,0,N,K),shu[sl])
           record((geo,'shared_KL_matched',N,0,N,K),matched[sl])
          for f in P['calibration_fractions']:
            ...
            for method,u in [('paired_KL',pu),('separate_KL',su)]:
             for local,geo in enumerate(ids):
              ...
              record(key,u[3*local:3*local+3])
      if (rep+1)%10==0:
    ...
    for key,acc in counts.items():
      ...
      for j,x in enumerate(acc):rows.append(dict(key=list(key),geometry_id=geo,geometry=GEOS[geo],method=method,total_anchors=N,m=m,n=n,K=K,evaluations=N*K,candidate=j,accepted=int(x),replications=reps,power=float(x/reps),ci=wilson(int(x),reps),coverage=float(covered[key][j]/reps),family_false_acceptance=float(family_false[key]/reps),family_false_ci=wilson(family_false[key],reps),mean_width=float(width_sum[key][j]/reps),sd_width=float(np.sqrt(max(0,(width_sq[key][j]-width_sum[key][j]**2/reps)/(reps-1))))))
