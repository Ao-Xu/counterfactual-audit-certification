...
LEDGER = {}
REPAIR_VERSION = '2026-09-24-r3-seed-ci'
...
def hans():
    # r3: reuse the exact frozen CSV already used by r2; do not refresh inputs.
    src=ROOT/'experiments/hans_suite/outputs/repair_data/results.csv'
    source_hash=sha(src)
    old_ledger=json.loads((QA/'repair_manifest.json').read_text(encoding='utf8'))
    for fig_name in ['fig_hans_main.pdf','fig_hans_external.pdf']:
        entry=old_ledger['experiments/hans_suite/outputs/'+fig_name]
        assert entry['sources'][0]['sha256']==source_hash,'Frozen HANS CSV changed'
    df=pd.read_csv(src)
    ...
    seeds=sorted(df.seed.unique().tolist());assert len(seeds)==5
    assert not df.duplicated(['condition','seed','split']).any()
    sub=df[df.split.isin(['hans_'+g for g in groups])].copy()
    sub['group']=sub.split.str.replace('hans_','',regex=False)
    assert len(sub)==5*5*3
    assert (sub.groupby(['condition','seed']).group.nunique()==3).all()
    assert not sub[['risk','nll']].isna().any().any()
    tcrit=2.776445;intervals=[];plot_limits=[]

    def interval(values,mean,old_half,figure,panel,condition,metric,group=None,old_n=5):
        """Keep the displayed mean; replace only the uncertainty calculation."""
        values=np.asarray(values,dtype=float)
        assert len(values)==5 and np.isfinite(values).all()
        calculated=float(values.mean());assert np.isclose(mean,calculated,rtol=0,atol=1e-14)
        sd=float(values.std(ddof=1));half=tcrit*sd/np.sqrt(5)
        intervals.append({'figure':figure,'panel':panel,'condition':condition,'metric':metric,'group':group,
            'seeds':seeds,'seed_values':values.tolist(),'n_seeds':5,'df':4,'t_critical':tcrit,
            'sample_sd_ddof1':sd,'mean':float(mean),'seed_mean':calculated,
            'mean_difference':calculated-float(mean),'half_width':float(half),
            'lower95':float(mean-half),'upper95':float(mean+half),
            'previous_half_width':float(old_half),'previous_n_records':old_n,
            'previous_method':'normal 1.96*SE'+(' over pooled seed-heuristic rows' if old_n==15 else ' over seeds')})
        return half

    def verify_visible(axes,figure,panels,directions):
        for ax,panel,direction in zip(axes,panels,directions):
            limits=ax.get_ylim() if direction=='y' else ax.get_xlim()
            records=[r for r in intervals if r['figure']==figure and r['panel']==panel]
            low=min(r['lower95'] for r in records);high=max(r['upper95'] for r in records)
            assert limits[0]<=low and high<=limits[1],figure+panel+' interval clipped'
            plot_limits.append({'figure':figure,'panel':panel,'uncertainty_axis':direction,
                'axis_limits':list(limits),'minimum_lower95':low,'maximum_upper95':high,
                'all_endpoints_within_axes':True})
    ...
    for j,cond in enumerate(order):
        ...
        errors=[interval(s[s.group==g].set_index('seed').reindex(seeds).risk.to_numpy(),
            mean[g],1.96*e[g],'F25','a',cond,'risk',group=g) for g in groups]
        axs[0].errorbar(range(3),mean,yerr=errors,marker=MARKS[j],color=colors[j],ls='none',capsize=2.5,ms=4,label=fullnames[j])
    ...
    for ax,metric,letter,label in [(axs[1],'risk','b','Mean HANS Brier risk'),(axs[2],'nll','c','Mean HANS NLL')]:
        # The previous pooled mean is numerically equal to the seed-mean average,
        # because all five seeds have exactly the same three heuristic groups.
        # Retain that displayed mean bit-for-bit; do not retain the pooled SE.
        g=sub.groupby('condition')[metric].agg(['mean','std','count']).reindex(order)
        byseed=sub.groupby(['condition','seed'])[metric].mean()
        err=[interval(byseed.loc[c].reindex(seeds).to_numpy(),g.loc[c,'mean'],
            1.96*g.loc[c,'std']/np.sqrt(g.loc[c,'count']),'F25',letter,c,metric,old_n=15) for c in order]
        ...
        ax.invert_yaxis()
    for ax in axs:axes_style(ax)
    fig.text(.5,.02,'95% t intervals across 5 seeds (df=4); (b,c): within-seed heuristic means',ha='center',fontsize=7.5)
    verify_visible(axs,'F25',['a','b','c'],['y','x','x'])
    finish(fig,'experiments/hans_suite/outputs/fig_hans_main.pdf',[src],
        ['Three original panels retained in wide-top + two-bottom layout (the supplied HANS PDF has three, not four panels).',
         'r3 parent-authorized statistical correction: all intervals use t=2.776445, df=4 over five independent seeds.',
         'Panels b/c first average the three heuristics within each seed; pooled15-record uncertainty removed. Displayed means unchanged.',
    ...
    for split,label,col,off,hatch in [('hans_all','HANS',GREEN,-w/2,''),('anli_dev_r123','ANLI dev',PURPLE,w/2,'///')]:
        ...
        assert (g['count']==5).all()
        errors=[interval(df[(df.split==split)&(df.condition==c)].set_index('seed').reindex(seeds).brier.to_numpy(),
            g.loc[c,'mean'],1.96*g.loc[c,'std']/np.sqrt(5),'F26','a',c,'brier',group=split) for c in order]
        axs[0].bar(x+off,g['mean'],w,yerr=errors,color=col,hatch=hatch,capsize=2,label=label)
    axs[0].set(xticks=x,xticklabels=names,ylabel='Brier risk (bounded)',title='(a) Covered versus external shift')
    external(axs[0],y=-.35)
    piv=df[df.split=='anli_dev_r123'].pivot(index='seed',columns='condition',values='brier').reindex(seeds)
    gaps=[(piv[c]-piv.factual).to_numpy() for c in order[1:]]
    errors=[interval(v,v.mean(),1.96*v.std(ddof=1)/np.sqrt(5),'F26','b',c,'paired_brier_gap') for c,v in zip(order[1:],gaps)]
    axs[1].bar(range(4),[v.mean() for v in gaps],yerr=errors,color=colors[1:],capsize=2)
    ...
    for ax in axs:axes_style(ax)
    fig.text(.5,.025,'95% t intervals across 5 seeds (df=4); (b): paired differences vs. factual',ha='center',fontsize=7.5)
    verify_visible(axs,'F26',['a','b'],['y','y'])
    finish(fig,'experiments/hans_suite/outputs/fig_hans_external.pdf',[src],
        ['All five conditions and displayed means unchanged; original frozen data, no retraining.',
         'r3: 95% t intervals, t=2.776445 and df=4 over five seeds; panel b preserves within-seed pairing to factual.'])
    # Independent parent cross-check; the third contrast is a provenance/table
    # check, not an extra curve or bar added to the original figure.
    parent_checks=[('hans_balanced','factual',(.02512252756,.0026571462,.0475879089)),
                   ('natural_cf','factual',(.00536963993,-.0161608087,.0269000886)),
                   ('reversed_corrupt','aligned_corrupt',(-.08889224346,-.1555939129,-.0221905741))]
    paired_checks=[]
    for candidate,reference,expected in parent_checks:
        values=(piv[candidate]-piv[reference]).to_numpy();mean=float(values.mean())
        half=tcrit*values.std(ddof=1)/np.sqrt(5)
        actual=[mean,float(mean-half),float(mean+half)]
        assert np.allclose(actual,expected,rtol=0,atol=1e-8),'Parent paired contrast mismatch'
        paired_checks.append({'candidate':candidate,'reference':reference,'seeds':seeds,
            'paired_seed_differences':values.tolist(),'mean':actual[0],'lower95':actual[1],
            'upper95':actual[2],'parent_rounded_mean_lower_upper':list(expected),
            'max_abs_difference_from_parent':float(np.max(np.abs(np.array(actual)-expected))),
            'matches_parent_within_1e-8':True})
    assert sha(src)==source_hash and len(intervals)==39
    validation={'repair_version':REPAIR_VERSION,'source_csv':src.relative_to(ROOT).as_posix(),
        'source_sha256_before':source_hash,'source_sha256_after':sha(src),'intervals':intervals,
        'n_intervals':len(intervals),'max_abs_mean_difference':max(abs(r['mean_difference']) for r in intervals),
        'plot_interval_visibility':plot_limits,
        'external_paired_contrast_checks':paired_checks,
        'displayed_means_preserved':True,'raw_data_changed':False,'retraining_performed':False,
        'statistics':'two-sided mean +/- 2.776445 * sample_sd(ddof=1) / sqrt(5); df=4',
        'F25_bc_aggregation':'unweighted mean of exactly three heuristics within seed, then t interval across five seeds',
        'F26_b_pairing':'condition minus factual within each seed before the five-seed t interval'}
    (QA/'hans_seed_ci_validation.json').write_text(json.dumps(validation,indent=2),encoding='utf8')
    for fig_name in ['fig_hans_main.pdf','fig_hans_external.pdf']:
        LEDGER['experiments/hans_suite/outputs/'+fig_name].update({
            'repair_version':REPAIR_VERSION,'uncertainty_unit':'independent training seed',
            'n_seeds':5,'degrees_of_freedom':4,'t_critical':tcrit,
            'statistical_validation':'build/revision/figure_audit/repairs/hans_seed_ci_validation.json',
            'displayed_means_preserved':True,'source_data_changed':False})
...
def dashboard():
    # Explicit parent approval, 2026-09-24: omit obsolete fixed-feature Thm3
    # MAE panel. Its original code, CSV and four-panel PDF/SVG remain archived.
    paths=['confirmatory_v1/lora_metrics.json',
    ...
    directions=json.loads(sources[0].read_text())['rows']
    inter=json.loads(sources[1].read_text());slack_record=json.loads(sources[2].read_text())
    slack=slack_record['qwen'];assert slack_record['n']==1600
    validation={'repair_version':REPAIR_VERSION,'paired_pool_intervals':[],
                'one_sided_endpoints':[],'slack_n':slack_record['n'],'slack':slack}
    fig=plt.figure(figsize=(5.5,6.25));gs=fig.add_gridspec(2,2,hspace=.82,wspace=.60,height_ratios=[1,.95])
    axs=[fig.add_subplot(gs[0,:]),fig.add_subplot(gs[1,0]),fig.add_subplot(gs[1,1])]
    fig.subplots_adjust(left=.14,right=.97,top=.94,bottom=.205)
    ax=axs[0];by={}
    ...
    for i,(name,col) in enumerate([('clean',BLUE),('random',GRAY),('oppose',GREEN),('reinforce',ORANGE)]):
        ...
        validation['paired_pool_intervals'].append({'condition':name,'n_pools':len(v),
            'values':v.tolist(),'mean':float(v.mean()),'lower95':float(v.mean()-half),
            'upper95':float(v.mean()+half),'t_multiplier':2.365})
        ax.errorbar(i,v.mean(),yerr=half,color='#222222',fmt='o',ms=4,mfc='white',capsize=2)
    ax.axhline(0,color=GRAY,ls='--',lw=.7)
    ax.set(xticks=range(4),xticklabels=['Clean','Random','Oppose','Reinforce'],ylabel='Action-risk change vs. factual',title='(a) Matched 20% LoRA corruptions')
    ax.text(.5,-.22,'8 paired pools; mean and 95% t interval\nClean is the uncorrupted control.',transform=ax.transAxes,ha='center',va='top',fontsize=8)
    ax=axs[1]
    ...
    for y,(name,record,scale,col) in enumerate(specs):
        ...
        assert upper>=mean
        validation['one_sided_endpoints'].append({'metric':name,'difference':record['difference'],
             'upper95':record['upper95'],'display_factor':scale,'lower95':None})
    ...
    ax.set(yticks=range(3),yticklabels=[s[0] for s in specs],ylim=(-.5,2.5),xlim=(-7.6,2),
           xlabel='Scaled contrast (negative improves)',title='(b) Prospective endpoints')
    ...
    ax=axs[2];xx=np.arange(len(slack));w=.25
    ...
    for r in slack:
        assert np.isclose(r['cancellation']*r['heterogeneity'],r['utilization'],rtol=1e-13,atol=1e-15)
    ax.set(xticks=xx,xticklabels=[str(r['alpha']) for r in slack],ylim=(0,1),xlabel='Head parameter α',ylabel='Ratio (unitless)',title='(c) Transfer slack\n1,600 frozen anchors')
    external(ax,ncol=1,y=-.32,fontsize=7.8)
    for ax in axs:axes_style(ax)
    finish(fig,'experiments/llm_cfpt/real_llm_evidence/fig_real_llm_evidence.pdf',sources,
        ['Parent-approved deletion of obsolete fixed-feature Thm3 MAE panel only; all three retained diagnostics and negative gates unchanged.',
         'Three-panel order: (a) matched20% LoRA8pool differences, (b) one-sided prospective endpoints, (c) transfer slack.',
         'Previous repaired four-panel script/PDF/SVG archived under repairs/history/dashboard_four_panel_repaired; original decision_summary.csv remains in repair_data.',
    ...
    (QA/'dashboard_validation.json').write_text(json.dumps(validation,indent=2),encoding='utf8')
...
def approve_reviewed(ids):
    ...
    record={'reviewed_at_utc':datetime.now(timezone.utc).isoformat(),'repair_version':REPAIR_VERSION,
            'repair_script_sha256':sha(Path(__file__)),'figures':checks,
...
def install_reviewed():
    ...
    priorfile=QA/'installed_manifest.json'
    prior=json.loads(priorfile.read_text(encoding='utf8')) if priorfile.exists() else {'figures':[]}
    priorrows={r['path']:r for r in prior['figures']}
    ...
    for r in rows:
        ...
        acceptable_pdf=[original[r['path']]['before_sha256'],r['sha256']]
        if r['path'] in priorrows:acceptable_pdf.append(priorrows[r['path']]['installed_pdf_sha256'])
        assert sha(dest) in acceptable_pdf,'Canonical changed by another writer: '+r['path']
        ...
        if dest.with_suffix('.svg').exists():
            ...
            if r['path'] in priorrows:acceptable.append(priorrows[r['path']]['installed_svg_sha256'])
            assert sha(dest.with_suffix('.svg')) in acceptable,'Unowned SVG exists: '+str(rel)
    installed=[];changed=[]
    for r in rows:
        dest=ROOT/r['path']
        for ext in ('.pdf','.svg'):
            staged=(STAGE/r['path']).with_suffix(ext);target=dest.with_suffix(ext)
            if not target.exists() or sha(target)!=sha(staged):
    ...
    (QA/'installed_manifest.json').write_text(json.dumps({'installed_at_utc':datetime.now(timezone.utc).isoformat(),
        'repair_version':REPAIR_VERSION,'repair_script_sha256':sha(Path(__file__)), 'changed_this_install':changed,
    ...
    print('VERIFIED',len(installed),'PDF + SVG pairs; changed',len(changed),'files; protected files unchanged during install',flush=True)
