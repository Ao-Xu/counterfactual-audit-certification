...
from pairing_analysis.analyze import band
from pairing_analysis.simulate import P,METHODS
R=Path(__file__).resolve().parent;O=R/'results';T=R.parent/'language_models/experiments/pairing_analysis';T.mkdir(parents=True,exist_ok=True)
...
text.extend([r'\begin{figure}[p]\centering\includegraphics[width=\textwidth]{experiments/pairing_analysis/path_independent.pdf}',r'\caption{Symmetric pairing/reuse factors and the log interaction. Confidence sets describe discrete grid thresholds, not continuous optima.}\end{figure}'])
