...
import numpy as np
from oral_attribution_v5.methods import radius
from oral_efficiency_v2.run import kl_upper
R=Path(__file__).resolve().parent;P=json.loads((R/'frozen_manifest.json').read_text());C=json.loads(Path('oral_shared_v4/frozen_manifest.json').read_text())['stress']
def run(phase):
